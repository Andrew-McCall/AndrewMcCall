...
const MORSE_MAP: Record<string, string> = Object.values(MORSE_GROUPS).reduce(
  (acc, group) => ({
    ...acc,
...
const ALL_CHARS = Object.keys(MORSE_MAP).map((key) => ({
  code: key,
  char: MORSE_MAP[key],
}));
...
class="text-2xl min-h-12 break-all text-green-50"></div>
...
`;
  ...
  const commit = () => {
    ...
    if ((isTraining || isChallenge) && currentTarget) {
      if (code === currentTarget.code) {
        ...
        if (isChallenge) scoreEl.textContent = score.toString();
  ...
  const backspace = () => {
    ...
    if (msgEl.textContent) msgEl.textContent = msgEl.textContent.slice(0, -1);
  };
