        ...
        ${
          note.tags.length
            ? `<div class="flex flex-wrap gap-1 mt-1">${note.tags
                .map(
                  (t) =>
                    `<span class="text-green-600 bg-green-900/30 rounded px-1.5 text-xs font-mono">${esc(t)}</span>`,
                )
                .join("")}</div>`
            : ""
        }`;
  ...
  const renderEditor = (note: Note | null) => {
    ...
    editorEl.innerHTML = `
      <input id="ed-title" type="text" placeholder="title" autocomplete="off" spellcheck="false"
        ...
        class="bg-stone-900 border border-green-900 focus:border-green-600 outline-none rounded px-3 py-2 text-green-200 text-lg placeholder-green-900 font-mono" />
      <textarea id="ed-body" placeholder="write here…" rows="14" spellcheck="false"
        class="bg-stone-900 border border-green-900 focus:border-green-600 outline-none rounded px-3 py-2 text-green-300 placeholder-green-900 font-mono resize-y">${esc(note?.body ?? "")}</textarea>
      ...
      <div class="flex flex-col gap-2">
        ...
        <div class="flex gap-2">
          <input id="ed-tag" type="text" placeholder="add a tag" autocomplete="off" spellcheck="false" list="tag-options"
            class="flex-1 bg-stone-900 border border-green-900 focus:border-green-600 outline-none rounded px-3 py-2 text-green-300 placeholder-green-900 font-mono text-sm" />
